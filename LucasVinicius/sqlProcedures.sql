...
IF EXISTS (SELECT name FROM master.dbo.sysdatabases WHERE name = 'Empresa')
...
/*
	...
	create Proc usp_datas
	@dtPed date
	as
	IF( SELECT COUNT(*) FROM PEDIDOS WHERE DataPed = @dtPed)>0
	BEGIN
	Select cl.Nome,p.DataPed,p.DataEntrega from Pedidos as p
	INNER JOIN Clientes AS cl On cl.CodCli = p.CodCli where P.DataPed = @dtPed
	END
	ELSE
	SELECT 'NADA OCORREU NESSA DATA' AS msg

	sELECT * FROM PEDIDOS

	
----------------------------------------------
create PROCEDURE usp_loginFunZ
	@codFun int,
	@telefone varchar(24)
	AS
	If(SELECT count(*) from Funcionarios where @codFun = CodFun AND @telefone = Fone)>0
	begin 
		Select 1
	end
	Else
...
create proc usp_deletar
@cat varchar(50)
as 
delete from Categorias where Descr = @cat 
select Descr from Categorias

drop proc usp_deletar
select * from Categorias
--------------------------------------------------------------------

drop proc usp_VerPed
Create procedure usp_VerPed
	@CodCli  char(5)
	as
		IF(select count(CodCli) from Pedidos where CodCli = @CodCli)>0
		begin

		Select prd.Descr, Cli.Nome, Cli.Endereco from Produtos as prd
	Inner Join DetalhesPed as dPed On
	dPed.CodProd = prd.CodProd
	INNER Join Pedidos as Ped On
	Ped.NumPed = dPed.NumPed
	INNER JOIN Clientes as Cli on
	Cli.CodCli = Ped.CodCli and Cli.CodCli = 2

		end
		else
		select 'Esse codigo não foi registrado a nenhuma compra' as msg_ERRO

-------------------------------------------
create procedure usp_InsertCateg

	@Descr  varchar(15)
	
	as
	if(select count(*) from categorias where Descr = @Descr)= 0
	begin 
	...
	ELSE
	BEGIN 
		Select 'aparentemente já existe uma categoria com essa Descrição' as msg_ERRO
	END

------------------
drop proc usp_partFunc

create proc usp_partFunc
	@CodFun int
	as
		IF(SELECT COUNT(*) FROM FUNCIONARIOS WHERE CodFun= @CodFun)>0
			BEGIN
			
			Select fun.Nome,Fun.Sobrenome,Fun.Cargo,count(*) as 'Qtde de pedidos feitos ' from Produtos as prd
	Inner Join DetalhesPed as dPed On
	dPed.CodProd = prd.CodProd
	INNER Join Pedidos as Ped On
	Ped.NumPed = dPed.NumPed
	INNER JOIN Funcionarios as Fun on
	Fun.CodFun = Ped.CodFun  where Fun.CodFun = 6
	Group by Fun.Nome,Fun.sobrenome,Fun.Cargo
			END
			else

			SELECT 'erro, esse codigo não foi encontrado' as msg
-------------------
...
select * from Categorias
insert into Categorias (Descr) values ('caca')




DELETE FROM Categorias WHERE Descr = 'caca'
...
Select prd.Descr, Cli.Nome, Cli.Endereco from Produtos as prd
	...
	Ped.NumPed = dPed.NumPed
	INNER JOIN Clientes as Cli on
	Cli.CodCli = Ped.CodCli and Cli.CodCli = '1'
